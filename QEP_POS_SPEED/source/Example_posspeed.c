...
void  POSSPEED1_Init(void)
{
	...
	EQep2Regs.QEPCTL.bit.PCRM=00;		// PCRM=00 mode - QPOSCNT reset on index event 01 mode reset when count overflows
	...
	EQep2Regs.QCAPCTL.bit.CEN=0; 		// QEP Capture Enable
...
void POSSPEED1_Calc(POSSPEED1 *p)
{
     long tmp1;
     unsigned int pos16bval1,temp2;
   	 _iq Tmp2,newp1,oldp1;

//**** Position calculation - mechanical and electrical motor angle  ****//     
     p->DirectionQep = EQep2Regs.QEPSTS.bit.QDF;    // Motor direction: 0=CCW/reverse, 1=CW/forward

	 pos16bval1=(unsigned int)EQep2Regs.QPOSCNT;     // capture position once per QA/QB period
     p->theta_raw = pos16bval1+ p->cal_angle;        // raw theta = current pos. + ang. offset from QA
	 
	 // The following lines calculate p->theta_mech ~= QPOSCNT/mech_scaler [current cnt/(total cnt in 1 rev.)]
	 // where mech_scaler = 4000 cnts/revolution
     tmp1 = (long)((long)p->theta_raw*(long)p->mech_scaler);  	// Q0*Q26 = Q26 
     tmp1 &= 0x03FFF000;                                        
     p->theta_mech = (int)(tmp1>>11);         		// Q26 -> Q15 
     p->theta_mech &= 0x7FFF;                       
...
// Unit Timer is configured for 100Hz in INIT function
	...
	{ 
		/** Differentiator	**/
		// The following lines calculate position = (x2-x1)/4000 (position in 1 revolution)
	 	pos16bval1=(unsigned int)EQep2Regs.QPOSLAT;	              // Latched POSCNT value		
     	tmp1 = (long)((long)pos16bval1*(long)p->mech_scaler);  	  // Q0*Q26 = Q26 
     	tmp1 &= 0x03FFF000;
     	tmp1 = (int)(tmp1>>11);         			                  // Q26 -> Q15 
     	tmp1 &= 0x7FFF;
		newp1=_IQ15toIQ(tmp1);
		oldp1=p->oldpos;

   		if (p->DirectionQep==0)      				// POSCNT is counting down
   		{
    		if (newp1>oldp1)
      			Tmp2 = - (_IQ(1) - newp1 + oldp1);    // x2-x1 should be negative
    		else
      		Tmp2 = newp1 -oldp1;
   		}
   		else if (p->DirectionQep==1)      			// POSCNT is counting up
   		{
    		if (newp1<oldp1)
      		Tmp2 = _IQ(1) + newp1 - oldp1;
    		else 
      		Tmp2 = newp1 - oldp1;                     // x2-x1 should be positive
   		}
...
//**** Low-speed computation using QEP capture counter ****//	
	if(EQep2Regs.QEPSTS.bit.UPEVNT==1)                 // Unit position event
	{
		if(EQep2Regs.QEPSTS.bit.COEF==0)               // No Capture overflow
			temp2=(unsigned long)EQep2Regs.QCPRDLAT;   // temp1 = t2-t1  
		else							               // Capture overflow, saturate the result
			temp2=0xFFFF;
	
		p->Speed_pr = _IQdiv(p->SpeedScaler,temp2);    // p->Speed_pr = p->SpeedScaler/temp1 
		Tmp2=p->Speed_pr;
